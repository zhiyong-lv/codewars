...
public class WhitespaceInterpreter {
	 ...
	 */
	private static char paringInputChars(BufferedReader is) throws Exception {
		char c = (char) is.read();
		if(-1 == c || c >= 256) throw new Exception();
		return c;
	}
	 ...
	 * @throws Exception 
	 */
	private static int paringInputNum(BufferedReader is) throws Exception {
		String numStr = is.readLine();
		if(null == numStr) {
			throw new Exception();
		}
		try{
	 ...
	 * @throws EmptyStackException should be thrown if there are not enough items on the stack to complete an operation.
	 */
	private static void io(String code, Stack<Integer> stack, Map<Integer, Integer> heap, BufferedReader in, OutputStream out) throws Exception {
		Matcher m = null;
	...
	public static String execute(String code, InputStream input) throws Exception {
		...
		String s = execute(code, input, out);
		out.close();
		return s;
	}
	...
	public static String execute(String code, InputStream input, OutputStream output) throws Exception {
		...
		while (m.find()) {
			...
			//Add unknown code to code section.
			if(m.start() != lastIndex) {
				String unknownCode = code.substring(lastIndex, m.start());
				codeSection.add(unknownCode);
				addr++;
			}
			
			// Add code to code section. code section pointer add 1.
			lastIndex = m.end();
			s = m.group();
			codeSection.add(s);
			addr++;
		...
		try {
			...
			executeUnderFlowControl(codeSection, nameSpace, stack, heap, input, baos, labelRepeatList, stopAddr);
		} catch (Exception e) {
			// flush output stream before throw a exception.
			flushOutStream(output, baos);
			throw e;
		} 
		
		// flush output stream before return string;
		flushOutStream(output, baos);
		
		return ((OutputStream)baos).toString();
	}
	
	private static void flushOutStream(OutputStream output, ByteArrayOutputStream baos) throws IOException {
		if(null == output) output = new ByteArrayOutputStream();
	...
	private static void executeUnderFlowControl(List<String> codeSection, Map<String, Integer> nameSpace,
		...
		Stack<Integer> progStack = new Stack<>();
		Set<String> labels = new HashSet<>();
		int pointer = 0;
		String code = "";
		BufferedReader is = (null != input) ? new BufferedReader(new InputStreamReader(input)) : null;
		
		while(pointer<codeSection.size() && pointer >= 0) {
			if(stopAddr == pointer) {
				throw new Exception();
			}
			code = codeSection.get(pointer++);
		...
		if(!progStack.isEmpty() || !code.equals("nnn")) {
			throw new Exception();
		}
